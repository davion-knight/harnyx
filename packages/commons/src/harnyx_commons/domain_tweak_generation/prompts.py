...
def reference_answer_prompt(candidate: DomainTweakQuestionCandidate, *, timestamp: datetime) -> str:
    return (
        ...
        "- Every non-obvious or search-dependent claim in reference_answer.text must be supported by a citation "
        "object whose note directly says what the citation supports.\n"
        "- Cover every requested entity, date, filter, comparison side, calculation operand, exclusion, and "
        "output ordering.\n"
        "- For list-all questions, include completeness evidence. Discuss excluded or borderline cases only when "
        "they are necessary to disambiguate the final answer or prevent a likely wrong alternative.\n"
        ...
        "7. Write reference_answer.text only after all load-bearing claims have source support.\n\n"
        "Return JSON with shape:\n"
