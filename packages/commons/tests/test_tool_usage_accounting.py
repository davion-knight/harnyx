...
def test_merge_tool_usage_summaries_combines_provider_model_usage() -> None:
    ...
    assert merged.reference_total_cost_usd == pytest.approx(1.0)
    assert merged.reference_cost_by_provider["vertex"] == pytest.approx(1.0)
    assert merged.actual_total_cost_usd == pytest.approx(0.94)
    assert merged.actual_cost_by_provider["vertex"] == pytest.approx(0.94)
...
def _usage_summary(
    *,
    ...
    if actual_llm_cost is not None or actual_search_cost is not None:
        actual_total = (actual_llm_cost or 0.0) + (actual_search_cost or 0.0)
    ...
    return ToolUsageSummary(
        ...
        llm_cost=llm_cost,
        reference_total_cost_usd=llm_cost + search_cost,
        reference_cost_by_provider={provider: llm_cost + search_cost},
